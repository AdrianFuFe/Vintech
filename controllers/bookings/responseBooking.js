...
async function responseBooking (req,res,next){
    ...
    try {
        ...
        if(booking == false) throw new Error (`No existe la reserva con ID ${idBooking}`);

        console.log(booking.status);
        
        //comprobamos si que el usuario es el vendedor 
        if(booking.id_user_A !== Number(id)) throw new Error (`Solo el vendedor con ID ${booking.id_user_A} puede responder a esta reserva`);
        ...
        if (response === "accept"){
            ...
            if(!meetDate || !ubication) throw new Error (`Debes establecer una fecha y un lugar`);
