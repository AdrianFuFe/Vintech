...
async function acceptBooking(req, res, next) {
  ...
  try {
    ...
    if (result == false)
      throw new Error(`El producto con ID ${idProduct} no existe`);
    //comprobamos que el producto no este ya reservado
    if (result.status != "active")
      throw new Error(`El producto con ID ${idProduct} ya esta reservado`);
    ...
    if (result.length < 1)
      throw new Error(
        `No existe la reserva para el usuario vendedor con ID ${id} para el producto con ID ${idProduct}`
      );
